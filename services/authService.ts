...
export const loginUser = async (rawEmail: string, pass: string): Promise<User> => {
  ...
  try {
    ...
    if (!userProfile && email.toLowerCase() === 'zaid@talentlyx.com') {
    ...
    if (e.code === 'auth/user-not-found' && email.toLowerCase() === 'zaid@talentlyx.com') {
      try {
    ...
    if (e.code === 'auth/wrong-password' || e.code === 'auth/invalid-credential') {
      if (email.toLowerCase() === 'zaid@talentlyx.com') {
...
export const deleteUserProfile = async (email: string) => {
  if (email === 'zaid@talentlyx.com') throw new Error("Cannot delete Super Admin.");
