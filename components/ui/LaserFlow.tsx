...
export const LaserFlow = ({
    ...
    useEffect(() => {
        ...
        const dprClamp = (v, lo, hi) => Math.max(lo, Math.min(hi, v));
        const dprFloor = 0.6;
        ...
        const adjustDprIfNeeded = now => {
            ...
            if (avgFps < lowerThresh) {
                next = dprClamp(currentDprRef.current * 0.85, dprFloor, base);
            } else if (avgFps > upperThresh && currentDprRef.current < base) {
                next = dprClamp(currentDprRef.current * 1.1, dprFloor, base);
            }
